...
impl Authority {
    ...
    async fn
    update_ip_address(&self, domain: &str, name: &str, ip: &IpAddr, ttl: u32, record_type: RecordType)
                      -> Result<(), Error> {
        let _ = self.http_client
            .put(&(
                self.api_url.to_string() +
                    "/v1/domains/" + domain + "\
                    /records/" + &record_type.to_string() + "\
                    /" + &name + ""))
            .json(&[DnsRecord::new(
                name,
                record_type,
    ...
    pub async fn get_ipv4_address(&self, domain: &str, name: &str) -> Result<Ipv4Addr, Error> {
        ...
        let ipv4 =
            if let IpAddr::V4(result) = result { result } else { unreachable!() };
        return Ok(ipv4);
    }
    ...
    pub async fn get_ipv6_address(&self, domain: &str, name: &str) -> Result<Ipv6Addr, Error> {
        ...
        let ipv6 =
            if let IpAddr::V6(result) = result { result } else { unreachable!() };
        return Ok(ipv6);
    }

    async fn get_address(&self, domain: &str, name: &str, record_type: RecordType)
                         -> Result<IpAddr, Error> {
